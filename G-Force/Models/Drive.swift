...
struct Drive: Identifiable, Codable, Hashable {
    ...
    /// PROVISIONAL comfort score: RMS as a baseline, with an extra penalty scaled
    /// to how far peak G exceeds RMS (a crest-factor penalty, so occasional harsh
    /// spikes hurt more than a uniformly-mediocre drive). Lower is better, same
    /// direction as raw g-force. This is a first guess, not the final model —
    /// see SkillTier.swift for the same caveat on tier thresholds.
    var score: Double {
        rmsG + 0.5 * max(0, peakG - rmsG)
    }
}
